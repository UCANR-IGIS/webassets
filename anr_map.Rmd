...
leafshared_ucce <- leaflet(ucce_shared, width=530, height=640) %>% 
          fitBounds(ca_bb[1,1], ca_bb[2,1], ca_bb[1,2], ca_bb[2,2]) %>% 
          addTiles("//{s}.tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png", 
                   group="OpenStreetMap BW", attribution=attr_osm) %>%
          addTiles("//{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", 
                   group="OpenStreetMap", attribution=attr_osm) %>%
          addTiles("//{s}.tile.opentopomap.org/{z}/{x}/{y}.png", 
                   group="OpenTopoMap", attribution=attr_otm) %>%
          addTiles("//server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}.png",
                   group="ESRI World Imagery", attribution=attr_esri) %>%
          addLayersControl(baseGroups = c("OpenStreetMap BW", "OpenStreetMap", "OpenTopoMap", "ESRI World Imagery"),
                   options = layersControlOptions(collapsed = FALSE)) %>%
          addCircleMarkers(~lon, ~lat, label=~htmltools::htmlEscape(name), 
                           popup=popup_txt[idx_ucce],
                           radius=6, 
...
ANR’s nine field stations are located throughout California's diverse agricultural regions and ecosystems. RECs support research projects and public education events to deliver the highest-quality science to growers, industry, and land managers.
...
## Create the leaflet object and add tiles
leafshared_rec <- leaflet(rec_shared, width=530, height=640) %>% 
          fitBounds(ca_bb[1,1], ca_bb[2,1], ca_bb[1,2], ca_bb[2,2]) %>% 
          addTiles("//{s}.tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png", 
                   group="OpenStreetMap BW", attribution=attr_osm) %>%
          addTiles("//{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", 
...
## Create the leaflet object and add tiles
leafshared_aes <- leaflet(aes_shared, width=530, height=640) %>% 
          fitBounds(ca_bb[1,1], ca_bb[2,1], ca_bb[1,2], ca_bb[2,2]) %>% 
          addTiles("//{s}.tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png", 
                   group="OpenStreetMap BW", attribution=attr_osm) %>%
          addTiles("//{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", 
